...
describe('LoginForm', () => {
  it('requests OTP only after the user submits a mobile number', async () => {
    ...
    await waitFor(() => expect(fetchMock).toHaveBeenCalledWith('/api/auth/login', expect.objectContaining({ method: 'POST', body: JSON.stringify({ mobileNumber: '9876543210' }) })));
    expect(screen.getByLabelText('Verification code')).toBeInTheDocument();
  });

  it('shows a backend verification error for an incorrect OTP', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true }).mockResolvedValueOnce({ ok: false, json: async () => ({ error: { message: 'The verification code is incorrect.' } }) });
    render(<LoginForm />);
    fireEvent.change(screen.getByLabelText('Mobile number'), { target: { value: '9876543210' } });
    fireEvent.click(screen.getByRole('button', { name: 'Continue' }));
