...
export function OtpForm({ mobileNumber, onStartOver }: OtpFormProps): React.JSX.Element {
  ...
  /** Submits the verification code and proceeds only after the backend accepts it. */
  ...
  return (
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      <p className="verification-copy">
        We sent a code for <strong>{mobileNumber}</strong>. For local development, use{' '}
        <strong>1234</strong>.
      </p>
      ...
      {error && (
        <p id="otp-error" className="form-error" role="alert">
          {error}
        </p>
      )}
    </form>
